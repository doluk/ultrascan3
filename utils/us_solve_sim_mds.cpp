...
#define DbgLv(a) if(dbg_level>=a)qDebug()<<"SM-w:"<<thrnrank<<":"
#endif
...
// Band-forming threshold controls.  These live in us_solve_sim.cpp and are
// shared: they are read from etc/bandform.config by whichever solver is
// constructed first, and both read the same file.
extern double zerothr;     //!< zero threshold OD value
extern double linethr;     //!< linear threshold OD value
extern double maxod;       //!< maximum OD value
extern double mfactor;     //!< peak multiplier value
extern double mfactex;     //!< peak multiplier - experiment
extern double minnzsc;     //!< minimum non-zero scale factor

// Create a multi-data-set Solve-Simulation object
US_SolveSimMDS::US_SolveSimMDS( QList< DataSet* >& data_sets, int thrnrank,
...
//
// The test is a property of the data sets and the model, not of the solver,
// so it is not duplicated here.
bool US_SolveSimMDS::check_grid_size( double s_max, QString& smsg )
{
   return US_SolveSim::checkGridSize( data_sets, s_max, smsg );
}

// Do the real work of a thread/processor:  simulation from a solutes set,
// over a series of data sets each carrying its own systematic noise
void US_SolveSimMDS::calc_residuals( int offset, int dataset_count, Simulation& sim_vals,
...
if(thrnrank<2) DbgLv(1) << "CR:zthr lthr mxod mfac"
 << zerothr << linethr << maxod << mfactor;
   ...
   // Map where each data set's block sits in the concatenated vectors.  Every
   // routine in the noise algebra works from this table, so none of it is
   // tied to a single data set.  This also totals the counts the old loop did.
   build_geometry( offset, dataset_count );
   ...
   if ( ! dsgeom.isEmpty() )
   {  // Leave these where the counting loop used to leave them
      nscans        = dsgeom.last().nscans;
      npoints       = dsgeom.last().npoints;
   }
   //----------------------------------------------------------
...
DbgLv(1)<< " norm cuts for #solutes=" << ksolutes << ":  count_cut" << count_cut;
   ...
   // Columns of "A" are narows apart, which is longer than the data when
   // regularization pads each one.  The noise routines index A by column, so
   // they need the real stride, not the data length.
   na_stride    = narows;
...
DbgLv(1) << "entering for saving";
      ...
      {  // Return A and B with padding for regularization
         ...
         if ( calc_ri || calc_ti )
         {
            ...
            for ( int ii = 0; ii < dsgeom.size(); ii++ )
            {  // Add back each data set's own noise, over its own block
               const DsGeom& gm = dsgeom[ ii ];

               for ( int ss = 0; ss < gm.nscans; ss++ )
               {
                  double rinoi = calc_ri ? rinvec[ gm.rioffs + ss ] : 0.0;

                  for ( int rr = 0; rr < gm.npoints; rr++ )
                  {
                     double tinoi = calc_ti ? tinvec[ gm.tioffs + rr ] : 0.0;
...
// Set abort flag
void US_SolveSimMDS::abort_work()
{
   abort = true;
}

// Map out where each data set's block sits in the concatenated fit vectors
//
// "B" and each column of "A" run scan-major within a data set and data-set-
// major overall.  The TI vectors carry one value per radial point of each
// data set and the RI vectors one value per scan, concatenated in the same
// data-set order.  A set's three offsets are therefore three different
// numbers, and every routine in the noise algebra needs all three.
//
// Band-forming substitutes a thresholded copy of the data, but it is a copy
// of the same grid, so the geometry is unchanged.
void US_SolveSimMDS::build_geometry( int offset, int dataset_count )
{
   int lim_offs = offset + dataset_count;
   int toffs    = 0;
   int tioffs   = 0;
   int rioffs   = 0;

   dsgeom.clear();
   dsgeom.reserve( dataset_count );

   for ( int ee = offset; ee < lim_offs; ee++ )
   {
      US_DataIO::EditedData* edata = &data_sets[ ee ]->run_data;
      DsGeom                 gm;
      gm.npoints   = edata->pointCount();
      gm.nscans    = edata->scanCount();
      gm.toffs     = toffs;
      gm.tioffs    = tioffs;
      gm.rioffs    = rioffs;
      dsgeom << gm;

      toffs       += ( gm.nscans * gm.npoints );
      tioffs      += gm.npoints;
      rioffs      += gm.nscans;
   }
   ...
   // Until the A matrix is built, a column is as long as the data.  If
   // regularization pads each column, calc_residuals() updates this.
   na_stride    = nt_total;
}

// Compute a_tilde, the average experiment signal at each time
//
// The average is over the radial points of the scan's own data set: a scan of
// data set 1 knows nothing about the radial grid of data set 0.
void US_SolveSimMDS::compute_a_tilde( QVector< double >& a_tilde,
                                      const QVector< double >& nnls_b )
{
   for ( int ii = 0; ii < dsgeom.size(); ii++ )
   {
      const DsGeom& gm = dsgeom[ ii ];
      double avgscale  = 1.0 / (double)gm.npoints;
      int    jb        = gm.toffs;
      int    jt        = gm.rioffs;
      ...
      for ( int ss = 0; ss < gm.nscans; ss++ )
      {
         ...
         a_tilde[ jt++ ]  = sum * avgscale;
      }
   }
}

// Compute L_tildes, the average signal at each radius
void US_SolveSimMDS::compute_L_tildes( int                      nrinois,
                                       int                      nsolutes,
                                       ...
                                       const QVector< double >& nnls_a )
{
   for ( int cc = 0; cc < nsolutes; cc++ )
   {
      int a_base       = cc * na_stride;
      int t_base       = cc * nrinois;

      for ( int ii = 0; ii < dsgeom.size(); ii++ )
      {
         const DsGeom& gm = dsgeom[ ii ];
         double avgscale  = 1.0 / (double)gm.npoints;
         int    a_index   = a_base + gm.toffs;
         int    t_index   = t_base + gm.rioffs;
...
// Compute L_tilde, the average model signal at each radius
void US_SolveSimMDS::compute_L_tilde( QVector< double >&       L_tilde,
                                      const QVector< double >& L )
{
   for ( int ii = 0; ii < dsgeom.size(); ii++ )
   {
      ...
      for ( int ss = 0; ss < gm.nscans; ss++ )
      {
         ...
         L_tilde[ jt++ ]  = sum * avgscale;
      }
   }
}

// Sum( concentration * Lamm ) over the whole series
void US_SolveSimMDS::compute_L( int                      ntotal,
                                int                      nsolutes,
                                QVector< double >&       L,
                                const QVector< double >& nnls_a,
                                const QVector< double >& nnls_x )
{
   for ( int cc = 0; cc < nsolutes; cc++ )
   {
      ...
      {
         int r_index = cc * na_stride;

         for ( int jj = 0; jj < ntotal; jj++ )
            L[ jj ]    += ( concentration * nnls_a[ r_index++ ] );
      }
   }
}

// Reduced normal equations with the per-data-set RI noise projected out
//
// The sums run over the whole series, but each element is measured against
// the mean of its own data set's scan.  For a balanced scan x point grid the
// scan-wise demeaning is an exact orthogonal projection, so solving this
// small system is the same as minimizing over the concentrations and every
// data set's RI vector at once.
void US_SolveSimMDS::ri_small_a_and_b( int                      nsolutes,
                                       int                      ntotal,
                                       int                      nrinois,
...
DebugTime("BEG:ri_smab");
   ...
   for ( int cc = 0; cc < nsolutes; cc++ )
   {
      ...
      for ( int ii = 0; ii < ndsets; ii++ )
      {
         const DsGeom& gm = dsgeom[ ii ];
         int    jjna      = cc * na_stride + gm.toffs;
         int    jjnb      = gm.toffs;
         int    jjat      = gm.rioffs;
         int    jjlt      = cc * nrinois + gm.rioffs;

         for ( int ss = 0; ss < gm.nscans; ss++ )
         {
            double atil  = a_tilde [ jjat++ ];
      ...
      //     L_tildes[ cc * nrinois + ss ] );
      //
      // The matrix is symmetric, so only one triangle is computed.
      for ( int kk = cc; kk < nsolutes; kk++ )
      {
         double sum_a = 0.0;
         ...
         {
            const DsGeom& gm = dsgeom[ ii ];
            int    jja1      = kk * na_stride + gm.toffs;
            int    jja2      = cc * na_stride + gm.toffs;
            int    jjt1      = kk * nrinois   + gm.rioffs;
            int    jjt2      = cc * nrinois   + gm.rioffs;
         ...
         small_a[ kk * nsolutes + cc ] = sum_a;
         small_a[ cc * nsolutes + kk ] = sum_a;
      }
...
// Reduced normal equations with the per-data-set TI noise projected out
//
// As for RI, but demeaned down each data set's own scans.  When RI noise is
// fitted too, a_bar and L_bars already carry the scan-wise demeaning, which
// makes this the two-way within transform of a balanced layout -- again exact
// per data set.
void US_SolveSimMDS::ti_small_a_and_b( int                      nsolutes,
                                       int                      ntotal,
                                       int                      ntinois,
                                       QVector< double >&       small_a,
                                       QVector< double >&       small_b,
...
DbgLv(1)<< "ti_small_a_and_b: nsolutes=" << nsolutes;
   ...
   for ( int cc = 0; cc < nsolutes; cc++ )
   {
      ...
      for ( int kk = cc; kk < nsolutes; kk++ )
      {
         ...
         {
            const DsGeom& gm = dsgeom[ ii ];
            int    jjna1     = kk * na_stride + gm.toffs;
            int    jjna2     = cc * na_stride + gm.toffs;
            int    jslb1     = kk * ntinois   + gm.tioffs;
            int    jslb2     = cc * ntinois   + gm.tioffs;
...
// Average simulated concentration at each radius, within each data set
void US_SolveSimMDS::compute_L_bar( QVector< double >&       L_bar,
                                    const QVector< double >& L,
                                    const QVector< double >& L_tilde )
{
   ...
   {
      const DsGeom& gm = dsgeom[ ii ];
      double avgscale  = 1.0 / (double)gm.nscans;
      ...
      for ( int rr = 0; rr < gm.npoints; rr++ )
      {
         ...
         for ( int ss = 0; ss < gm.nscans; ss++ )
            sum          += ( L[ gm.toffs + ss * gm.npoints + rr ]
                            - L_tilde[ gm.rioffs + ss ] );

         L_bar[ gm.tioffs + rr ] = sum * avgscale;
      }
   }
}

// Average measured concentration at each radius, within each data set
void US_SolveSimMDS::compute_a_bar( QVector< double >&       a_bar,
                                    const QVector< double >& a_tilde,
                                    const QVector< double >& nnls_b )
{
   for ( int ii = 0; ii < dsgeom.size(); ii++ )
   {
      ...
      for ( int rr = 0; rr < gm.npoints; rr++ )
      {
         ...
         for ( int ss = 0; ss < gm.nscans; ss++ )
         {
            sum          += ( nnls_b[ jb ] - a_tilde[ gm.rioffs + ss ] );
            jb           += gm.npoints;
         }

         a_bar[ gm.tioffs + rr ] = sum * avgscale;
      }
   }
}

// Average simulated concentration at each radius per solute, within each set
void US_SolveSimMDS::compute_L_bars( int                      nsolutes,
                                     int                      nrinois,
                                     int                      ntinois,
                                     int                      ntotal,
                                     ...
                                     const QVector< double >& L_tildes )
{
   for ( int cc = 0; cc < nsolutes; cc++ )
   {
      int solute_offset = cc * na_stride;
      ...
      for ( int ii = 0; ii < dsgeom.size(); ii++ )
      {
         ...
         for ( int rr = 0; rr < gm.npoints; rr++ )
         {
            int    r_index = cc * ntinois + gm.tioffs + rr;
            double sum     = 0.0;
            ...
            for ( int ss = 0; ss < gm.nscans; ss++ )
            {
               ...
               int n_index = solute_offset + gm.toffs + ss * gm.npoints + rr;
               int s_index = cc * nrinois  + gm.rioffs + ss;

               sum        += ( nnls_a[ n_index ] - L_tildes[ s_index ] );
            }

            L_bars[ r_index ] = sum * avgscale;
         }
      }
   }
}
// Debug message with thread/processor number and elapsed time value
void US_SolveSimMDS::DebugTime( QString mtext )
{
   if ( dbg_timing )
   {
...
bool US_SolveSimMDS::data_threshold( US_DataIO::RawData* sdata,
...
bool US_SolveSimMDS::data_threshold( US_DataIO::EditedData* edata,
      double zerothr, double linethr, double maxod, double mfactor )
{
   int    nnzro   = 0;
   int    npoints = edata->pointCount();
...
void US_SolveSimMDS::set_comp_attr( US_Model::SimulationComponent& component,
...
//-----------------------------------------
double US_SolveSimMDS::angle_vectors( QVector<double>& v1, QVector<double>& v2, int nn )
{
   double angle = 0.0;
   double sum1  = 0.0;
   double sum2  = 0.0;
...
double US_SolveSimMDS::angle_vectors( double* v1, double* v2, int nn )
{
   double angle = 0.0;
   double sum1  = 0.0;
   double sum2  = 0.0;
