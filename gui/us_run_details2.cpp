...
US_RunDetails2::US_RunDetails2( const QVector< US_DataIO::RawData >& data, 
   ...
   : US_WidgetsDialog( 0, Qt::WindowFlags() ), dataList( data ), triples( cell_ch_wl )
{
...
//*DEBUG*
   ...
   le_timeCorr->setText( wks.asprintf( "%d m %02d s", minutes, seconds ) );
   ...
   for ( int i = 0; i < triples.size(); i++ )
   {
      ...
      lw_triples->addItem( triples[ i ] + wks.asprintf( " -- %d scans", scans ) );
   ...
   lw_triples->addItem( wks.asprintf( "All scans -- %d scans", scanCount ) );
